...
void melhora_rotas(int Q ,int L, int *terc_size,int *clientes_att,int *total_cost, vector<int> d, vector<int> p, vector<vector<int>> c, vector<int> &v, int i, vector<int> &terc,int *rota_demanda){
    ...
    //cout << "Demanda do tercerizado " << terc[j] << ": " << d[terc[j]-1] << endl;
    //cout << "Demanda da rota: " << *rota_demanda << endl;
    //cout << "Clientes atendidos = " << *clientes_att << endl;
    int custo_manter_i = c[ant_i][v[i]] + c[v[i]][prox_i];

    if (*terc_size == 0){
        cout << "Nao tem terceirizado na rota" << endl;
    }
    ...
    if (*terc_size > 0){
        ...
        if (*rota_demanda - d[v[i]-1] + d[terc[j]-1] <= Q and terc_size > 0){
